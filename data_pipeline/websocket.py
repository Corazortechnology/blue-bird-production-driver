"""Real-time WebSocket frame pipeline — data ingestion, model inference, alerts."""
...
def _do_insert_alert(driver_id, session_id, alert_type, confidence_score):
    try:
        ...
        logger.debug("Alert inserted: driver=%s type=%s conf=%.2f", driver_id, alert_type, confidence_score)
...
class RealtimeFramePipeline:
    ...
    def _get_placeholder_jpeg(self) -> bytes:
        if self.placeholder_jpeg is None:
            ...
            ok, buf = cv2.imencode(".jpg", black, [int(cv2.IMWRITE_JPEG_QUALITY), JPEG_SEND_QUALITY])
    ...
    def process_frame(
        ...
        if run_models:
                ...
                if emb_3d is not None or emb_2d is not None:
                    try:
                        driver, _ = match_embedding_to_driver(
                            embedding_2d=emb_2d, embedding_3d=emb_3d, driver_id=driver_id,
                        )
            ...
            if frame_count % 30 == 0:
                logger.debug(
                    ...
                    frame_count, effective_state,
                    last_metrics["ear"], last_metrics["perclos"],
                    last_metrics["attention_state"], is_distracted,
                )
        ...
        ok, buf = cv2.imencode(".jpg", composite, [int(cv2.IMWRITE_JPEG_QUALITY), JPEG_SEND_QUALITY])
    ...
    async def stream(self, websocket: WebSocket) -> None:
        ...
        try:
            while not recv_task.done():
                ...
                try:
                    jpeg_bytes, alert_type, last_metrics = await loop.run_in_executor(
                        self.executor, self.process_frame,
                        data, run_models, frame_count, driver_id, recognition_result, last_metrics,
                    )
                except Exception as e:
                ...
                if alert_type and run_models:
                    ...
                    if effective_driver_id and session_id is None:
                        ...
                        logger.info("WebSocket session started: %s (driver=%s)", session_id, effective_driver_id)
                    loop.run_in_executor(
                        self.executor, _do_insert_alert,
                        effective_driver_id or "UNKNOWN", session_id,
                        alert_type, last_metrics.get("confidence_score", 0),
                    )
            ...
            recv_task.cancel()
            logger.info("WebSocket client disconnected (frames=%d, session=%s)", frame_count, session_id)
            if session_id:
